  ...
  return (
    <section>
      ...
      <div className="w-full bg-white p-[24px] flex flex-col lg:flex-row">
        <div className="flex-grow flex flex-col lg:flex-row items-center justify-between relative mb-8 lg:mb-0">
          {gameCards.map((game, index) => (
            <div
              key={index}
              className="flex flex-col items-center p-[9px] mb-12 lg:mb-0 relative z-10 rounded-[7.46px] gap-[9px]"
              style={{ boxShadow: '0px 0px 35.16px 0px #00000014' }}
            >
              <div className="relative max-w-[150px] h-[131.25px] rounded-[7.46px] overflow-hidden flex flex-col justify-center items-center">
              ...
              {index < 3 && (
                <>
                  <div
                    className="absolute left-full top-1/2 w-16 h-0.5 bg-gray-300 hidden lg:block"
                    style={{ backgroundImage: 'linear-gradient(to right, #D1D5DB 50%, transparent 50%)', backgroundSize: '8px 1px' }}
                  ></div>
                  <div className="absolute left-[calc(100%+32px)] top-1/2 -translate-y-1/2 w-[40px] h-[40px] bg-purple-500 rounded-full overflow-hidden flex items-center justify-center z-20 hidden lg:flex">
                    <img className="w-full h-full object-cover" src={game.imageSrc} />
                  </div>
                </>
              )}
